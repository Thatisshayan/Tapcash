        ...
        <section className="px-4 sm:px-6 lg:px-8 py-16" id="rewards">
          <div className="max-w-7xl mx-auto grid gap-6 lg:grid-cols-[0.9fr_1.1fr] items-start">
            <div className="space-y-4">
              <span className="text-[10px] uppercase tracking-[0.28em] text-slate-500 font-black">Rewards and payouts</span>
              <h2 className="text-3xl md:text-4xl font-black tracking-tight font-display text-slate-950">Offer-driven earnings with payout options people understand.</h2>
              ...
              <div className="flex flex-wrap gap-3">
                {payoutMethods.map((method) => (
                  <span key={method} className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-[0_12px_36px_rgba(15,23,42,0.04)]">
                    <Target className="w-4 h-4 text-[#1cd3b0]" />
                    {method}
                  </span>
                ))}
              </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {featureCards.map((card) => (
                <div key={card.title} className="rounded-[1.5rem] border border-black/8 bg-white p-5 shadow-[0_12px_36px_rgba(15,23,42,0.05)]">
                  <div className="flex items-center justify-between gap-4">
                    <h3 className="text-lg font-black text-slate-950">{card.title}</h3>
                    <ShieldCheck className="w-4 h-4 text-[#1cd3b0]" />
                  </div>
