...
export default function RapidoReachPage() {
  ...
  const trustPoints = [
    { title: "Signed session", description: "Only authenticated users can request the provider URL.", icon: Lock },
    { title: "Ledger-backed credits", description: "Reward completion still resolves on the server side.", icon: BadgeCheck },
    { title: "Fraud signals retained", description: "Audit data stays visible for verification and review.", icon: Activity },
  ];
        ...
        <MotionWrap>
          <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
            <div className="space-y-6">
              ...
              <PageShell eyebrow="Session model" title="Open the wall with a signed UID." description="If the URL cannot be signed, the page stays honest with an explicit fallback instead of pretending the wall is ready.">
                <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-[2rem] border border-white/8 bg-[linear-gradient(180deg,rgba(8,14,24,0.96),rgba(5,8,16,0.98))] overflow-hidden shadow-[0_30px_90px_rgba(0,0,0,0.38)]">
              ...
              <div className="min-h-[72vh]">
                {loading || fetching ? (
                  ...
                  <CTAButton href="/dashboard" label="Return to dashboard" />
                </div>
              </div>
            ) : error ? (
                  <div className="flex min-h-[72vh] items-center justify-center px-6 text-center">
                  ...
                  <iframe title="RapidoReach offerwall" src={iframeUrl} className="h-[72vh] w-full border-0 bg-[#040913]" allow="clipboard-write; fullscreen" />
                ) : (
                  <div className="flex min-h-[72vh] items-center justify-center px-6 text-center">
                    <div>
                      <Loader2 className="mx-auto h-10 w-10 animate-spin text-[#00e6c3]" />
