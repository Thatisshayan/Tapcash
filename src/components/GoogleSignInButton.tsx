  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      const refCookie = typeof document !== "undefined"
        ? document.cookie.split(";").find((c) => c.trim().startsWith("tapcash_ref="))?.split("=")[1]
        : undefined;
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        disabled={loading}
        className="w-full flex items-center justify-center gap-3 py-3.5 rounded-2xl border border-white/10 bg-white/4 text-white text-sm font-bold hover:bg-white/8 hover:border-white/20 transition disabled:opacity-60"
      >
        {loading ? (
